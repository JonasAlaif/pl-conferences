...
REGISTER_DESCRIPTION_Q="$PREFIX_Q Write a detailed description suitable for the DESCRIPTION field of an .ics (iCalendar) file. The text must contain all key information about applying to volunteer, including links or emails (if present) relevant to applying. The output must be formatted as a single line, using '\\\\n' for new lines. Ensure the output does not include the 'DESCRIPTION:' prefix."
